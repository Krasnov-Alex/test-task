...
begin
	...
	select
		...
		,cast(is null(cs.FlagActive, 0) as bit) as FlagActive
	...
	where try_cast(cs.DateBegin as date) is not null
		...
		and try_cast(is null(cs.FlagActive, 0) as bit) is not null
	...
	*/
	select
		...
		,case
			...
			when try_cast(is null(cs.FlagActive, 0) as bit) is null then 'Невозможно определить Активность'
		end as Reason
	...
	where cc.ID is null
		...
		or try_cast(is null(cs.FlagActive, 0) as bit) is null
